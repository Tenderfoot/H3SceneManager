...
From there you can either download the workflow JSON to run in ComfyUI yourself, or hit **Run Scene** and let Scene Forge submit every un-rendered sequence to a local ComfyUI instance in order, chaining each sequence's actual rendered output into the next one automatically.
...
Scene Forge doesn't run any generation itself — it produces workflow JSON for, and optionally talks to, your own ComfyUI instance. Your ComfyUI install needs:
...
- **[Workflow to API Converter Endpoint](https://github.com/SethRobinson/comfyui-workflow-to-api-converter-endpoint)** custom node, by Seth A. Robinson. **Required for the "Run Scene" automation feature.** ComfyUI's `/prompt` endpoint only accepts API-format workflow JSON, but the workflow Scene Forge generates is UI/"Save"-format (the format `template_engine.py` needs to work with, to duplicate groups and rewire nodes). This custom node adds a `/workflow/convert` endpoint that converts UI-format → API-format on the fly, which is what lets Scene Forge submit a generated workflow straight to ComfyUI without you manually re-saving it from the ComfyUI UI first. If you only ever plan to download workflow JSON and queue it yourself from the ComfyUI UI, you don't strictly need this — but you won't be able to use Run Scene without it.
- **A base template workflow** (`data/templates/Grant_Template_Workflow.json`) with `{{ }}`-marked template nodes/groups that Scene Forge duplicates per character. This ships with the project; you generally shouldn't need to touch it.

### Python

- Python 3, Flask. No database — everything is JSON files on disk under `data/`.
- Optional: [`websocket-client`](https://pypi.org/project/websocket-client/) (`pip install websocket-client`) — enables live, per-step render progress during a Run Scene job (reads ComfyUI's `/ws` progress events). Without it, Run Scene still works fully via REST polling alone; you just won't see a live percentage, only the current stage name (generating/converting/queued/rendering).
...
cd scene-forge
pip install flask
# optional, for live render progress:
pip install websocket-client

python3 app.py
```

Then open the app in your browser and go to the **Config** tab first to point Scene Forge at your ComfyUI instance:

| Setting | What it's for |
|---|---|
| ComfyUI URL | Where your ComfyUI instance is running (default `http://127.0.0.1:8188`) |
| ComfyUI output directory | **Must be the exact folder your ComfyUI install writes rendered video to.** Scene Forge reads this directly (same-machine, shared filesystem) to confirm a render landed and record its real path for chaining into the next sequence. |
| ComfyUI input directory | Where ComfyUI's `LoadImage`/`LoadAudio` nodes look for reference files. Scene Forge doesn't read/write here itself — it's just context for where your face/voice/location reference filenames should live. |
| Poll interval / render timeout | How often Scene Forge checks ComfyUI for job completion, and how long it'll wait before giving up on a stuck render. |
...
3. **Scenes** — pick a location, cast your characters (with a per-character "include voice reference" toggle), set a premise and visual style, then open the scene to build it out:
   ...
   - Prompt format (lean vs. full) and seed randomization are both chosen live, right before you generate or run — not saved to the scene, so you're free to mix formats sequence-to-sequence and compare results.
